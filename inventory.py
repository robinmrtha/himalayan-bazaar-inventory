...
def update_narf_flags():
            """
                NARF = North America Remote Fulfillment.
                    A CA/MX row is flagged as NARF when the SKU has an active listing in BOTH
                        the US AND that CA/MX marketplace. This means Amazon is fulfilling the
                            CA/MX orders directly from US FBA stock (cross-border remote fulfillment).
                                If the CA/MX listing is inactive or missing, it is not NARF — it is simply
                                    not selling in that market.
                                        """
    hdrs = {
                    "apikey":        SUPABASE_KEY,
    ...
    for r in all_rows:
        ...
        # NARF = active listing in US AND active listing in this CA/MX marketplace
        # Both must be active: US provides the stock, CA/MX is the storefront
        is_narf = (
                            sku in us_active
                            and listing == "active"
        )
