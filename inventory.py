...
def update_narf_flags():
    hdrs = {"apikey": SUPABASE_KEY, "Authorization": "Bearer " + SUPABASE_KEY, "Content-Type": "application/json"}
    res = requests.get(SUPABASE_URL + "/rest/v1/inventory?select=seller_sku,marketplace,fulfillable", headers=hdrs)
    all_rows = res.json()
    us_qty = {r["seller_sku"]: r["fulfillable"] for r in all_rows if r["marketplace"] == "US"}
    updated = 0
    for r in all_rows:
        if r["marketplace"] in ("CA", "MX"):
            sku = r["seller_sku"]
            is_narf = sku in us_qty and us_qty[sku] == r["fulfillable"] and r["fulfillable"] > 0
            requests.patch(
                SUPABASE_URL + "/rest/v1/inventory?seller_sku=eq." + requests.utils.quote(sku) + "&marketplace=eq." + r["marketplace"],
                headers=hdrs, json={"is_narf": is_narf}
            )
            updated += 1
...
if __name__ == "__main__":
    ...
    print("Fetching inventory (once for all markets)...")
    base_skus = get_inventory_once(token)
    print("Total SKUs in FBA: " + str(len(base_skus)))
    ...
    for country, marketplace_id in MARKETPLACES.items():
        ...
        try:
            ...
            listings = {}
        for sku, inv in base_skus.items():
            row = dict(inv)
